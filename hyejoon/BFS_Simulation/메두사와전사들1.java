 ...
 * 
 * - 문제 설명 
 * 도로는 0, 아닌 곳은 1 집 좌표는 Sr, Sc / 공원 좌표는 Er, Ec (항상 도로 위에 있음)
 * 도로만을 따라 최단경로로 공원까지 이동
 * 
 * M명의 전사들이 메두사를 잡고 각 전사들은 ri, ci에 위치 메두사를 향해 최단 경로로 이동
 * 전사들은 도로/비도로 구분하지 않고 이동함.
 * 
 ...
 * 
 * - 이동 순서 
 * 1. 메두사의 이동 : 최단경로로 가되, 이동한 칸에 전사가 있으면 전사 die 여러 최단경로가 가능하면 상-하-좌-우 의 순서를 따름 
 * * 공원까지 도달하는 경로가 없을 수도 있음 -1 출력
 * 2. 메두사의 시선 : 상,하,좌,우 하나의 방향을 선택해 바라봄. 90도의 시야각을 가질 수 있음. 
 * 전사는 다른 전사에 의해 가려질 수 있음 
 * - 메두사 기준 좌/ 우 나눠서 8방으로 시선에 닿는 전사들은 모두 돌로 변해 움직일 수 없음 가장 많은 전사를 볼 수 있는 방향으로 봄.
 * 만약 동이하다면 상하좌우 순서로 결정 
 * 3. 전사들의 이동 : 돌로 변하지않은 전사들은 메두사를 향해 최대 두 칸 이동.
 * * 첫번째 이동은 메두사와 거리를 줄일 수 있는 방향으로 한 칸이동 - 상하좌우 우선순위
 * 		격자 밖으로 이동 불가, 메두사의 시야에 들어오는 곳으로 이동 불가. 
 * * 두번째 이동은 메두사의 거리를 줄일 수 있는 방향으로 이동 -좌우상하 우선순위 동일하게 밖 이동 불가.
 * 		시야 이동 불가. 
 * 4. 전사의 공격 : 그렇지만 메두사는 죽지 않음.
 * 
 * -> 메두사가 공원에 도달할 때 까지 모든 전사가 이동한 거리의 합, 돌이 된 전사의 수, 메두사를 공격한 전사의 수를 출력 도착시 0 출력.
 * 
 * 필요한 메서드 
 * - medusaGoBfs : 메두사의 최단 경로 설정. 초기 한번만. * 우선순위 상하좌우 
 * - junsaDie1 : 메두사간 곳에 전사있으면 사라짐. 
 * - medusaSea : 메두사의 시선. * 우선순위 상하좌우 
 * - junsaMove1 : 전사 이동 *우선순위 상하좌우 
 * - junsaMove2 : 전사 이동 * 우선순위 좌우상하 
 * - junsaDie2 : 전사가 메두사에 도달시 사라짐.
 * 
 */
